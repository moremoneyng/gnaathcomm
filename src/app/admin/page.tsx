...
export default function AdminPage() {
  ...
  const resetProductForm = () => {
    setProductName('');
    setProductCategory('smartphones');
  ...
  const handleSaveProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        showToast(editingProduct ? 'Product updated!' : 'Product uploaded to Cloudinary & saved to DB!');
  ...
  const handleClearAllProducts = async () => {
    ...
    try {
      const res = await fetch('/api/products?clearAll=true', { method: 'DELETE' });
      const data = await res.json();
      if (data.success) {
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-3xl border border-slate-200 shadow-xs">
                <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
                  ...
                  <select
                    ...
                    className="w-full sm:w-auto px-3.5 py-2 bg-slate-50 border border-slate-300 rounded-xl text-xs font-semibold text-slate-800 focus:outline-none"
                  >
                    ...
                    <option value="creation">Content Creation Kits</option>
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto">
                  <button
                    ...
                    className="px-3.5 py-2.5 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 text-xs font-bold transition-colors flex items-center justify-center gap-2 w-full sm:w-auto"
                  >
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-extrabold transition-all shadow-md flex items-center justify-center gap-2 w-full sm:w-auto"
                  >
                      ...
                      {filteredProductsList.length > 0 ? (
                        ...
                        <tr>
                          <td colSpan={6} className="p-12 text-center text-slate-500 font-medium">
                            No products match filter. Click &quot;Upload &amp; Add New Product&quot; to add items to your catalog!
                          </td>
                        </tr>
                      )}
                    </tbody>

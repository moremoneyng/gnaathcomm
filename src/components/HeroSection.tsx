...
import { ShoppingBag, ArrowRight, ShieldCheck, Wrench, Sun, Smartphone, CheckCircle2 } from 'lucide-react';
...
export const HeroSection: React.FC = () => {
  ...
  return (
    <section className="relative overflow-hidden bg-[#f4f5f8] py-10 sm:py-20 lg:py-24 border-b border-slate-200/80">
      {/* Background Depth-of-Field Ambient Bokeh Orbs */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
        ...
        {/* Sleek Brand Tag (Hidden on Mobile) */}
        <div className="hidden sm:inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/90 backdrop-blur-md border border-slate-200/90 text-slate-800 text-xs font-bold shadow-sm mb-4 sm:mb-6">
        ...
        {/* Bolder, Bigger Mobile & Desktop Headline */}
        ...
        {/* Bolder, High-Legibility Subtitle */}
        ...
        {/* Motto Pill - Only "Ultimate Satisfaction Assured!" */}
        <div className="mt-4 sm:mt-5 inline-flex items-center px-4.5 py-2 rounded-full bg-gradient-to-r from-emerald-500/10 via-teal-500/15 to-emerald-500/10 border border-emerald-500/30 text-emerald-950 text-xs sm:text-sm font-extrabold shadow-sm backdrop-blur-xs">
          <span>&quot;{storeConfig.motto}&quot;</span>
        </div>

        {/* Bolder Action Pill Buttons */}
        <div className="mt-6 sm:mt-8 flex flex-row items-center justify-center gap-3 sm:gap-5">
          <button
            onClick={() => scrollToSection('catalog')}
            className="flex-1 sm:flex-initial inline-flex items-center justify-center gap-2 px-6 sm:px-8 py-3.5 sm:py-4 rounded-2xl sm:rounded-full font-extrabold text-xs sm:text-base bg-slate-950 hover:bg-slate-900 text-white shadow-xl hover:shadow-2xl active:scale-95 transition-all whitespace-nowrap"
          >
          ...
          <button
            ...
            className="flex-1 sm:flex-initial inline-flex items-center justify-center gap-2 px-6 sm:px-8 py-3.5 sm:py-4 rounded-2xl sm:rounded-full font-extrabold text-xs sm:text-base bg-white hover:bg-slate-50 text-slate-950 border border-slate-300/90 transition-all shadow-md hover:shadow-lg active:scale-95 whitespace-nowrap"
          >
        ...
        {/* Depth of Field Immersive Showcase (Hidden on Mobile) */}
        <div className="hidden sm:block mt-10 sm:mt-16 max-w-5xl mx-auto relative text-left">
          
          {/* Depth of Field Background Blur Layer */}
          ...
          {/* Main Showcase Canopy (Borderless Dark Glass Stage) */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 sm:gap-8 items-center relative z-10">
              ...
              <div className="md:col-span-7 space-y-4 sm:space-y-6">
                ...
                <div className="pt-2">
                  <button
                    onClick={() => scrollToSection('catalog')}
                    className="inline-flex items-center gap-2 px-6 py-3.5 rounded-xl sm:rounded-full text-xs sm:text-sm font-extrabold bg-emerald-500 hover:bg-emerald-400 text-slate-950 transition-all shadow-[0_10px_25px_rgba(16,185,129,0.3)] active:scale-95"
                  >
              ...
              {/* Right Column: Layered Depth-of-Field Image Showcase */}
              <div className="md:col-span-5 relative w-full h-60 sm:h-80 md:h-96">
                
                {/* Backlight Glow for Image Depth */}
                ...
                {/* Overlapping Floating Glass Badge Layer (Spatial Depth Effect) */}
        ...
        {/* Mobile Product Visual Showcase (Visible on Mobile Only - Clean & Uncluttered) */}
        <div className="block sm:hidden mt-8 relative max-w-sm mx-auto px-2">
          {/* Depth-of-Field Ambient Glow */}
          <div className="absolute -inset-3 bg-gradient-to-tr from-emerald-500/30 via-teal-400/25 to-cyan-500/30 rounded-3xl blur-2xl opacity-90 pointer-events-none -z-10" />

          {/* Main Mobile Product Image Container */}
          <div className="relative w-full h-64 rounded-3xl overflow-hidden border border-white/60 shadow-[0_20px_40px_rgba(0,0,0,0.25)] group">
            <Image
              src="https://images.unsplash.com/photo-1592750475338-74b7b21085ab?w=1000&auto=format&fit=crop&q=80"
              alt="Flagship Smartphones, Audio & Solar"
              fill
              className="object-cover group-hover:scale-105 transition-transform duration-700"
              priority
            />
        ...
        {/* Feature Grid Below (Desktop Only) */}
        <div className="hidden sm:grid mt-16 pt-8 border-t border-slate-200/80 grid-cols-2 md:grid-cols-4 gap-6 text-center max-w-4xl mx-auto">
          <div className="p-3.5 bg-white/90 backdrop-blur-sm rounded-2xl border border-slate-200/80 shadow-md hover:shadow-lg transition-all">
            <span className="text-xl font-black text-slate-950">Apple &amp; Samsung</span>
            <p className="text-xs text-slate-600 mt-0.5 font-bold">Original Smartphones</p>
          </div>
          <div className="p-3.5 bg-white/90 backdrop-blur-sm rounded-2xl border border-slate-200/80 shadow-md hover:shadow-lg transition-all">
            <span className="text-xl font-black text-slate-950">JBL &amp; Anker</span>
            <p className="text-xs text-slate-600 mt-0.5 font-bold">Authorized Accessories</p>
          </div>
          <div className="p-3.5 bg-white/90 backdrop-blur-sm rounded-2xl border border-slate-200/80 shadow-md hover:shadow-lg transition-all">
            <span className="text-xl font-black text-slate-950">Phone Repairs</span>
            <p className="text-xs text-slate-600 mt-0.5 font-bold">Screen &amp; Battery Fixes</p>
          </div>
          <div className="p-3.5 bg-white/90 backdrop-blur-sm rounded-2xl border border-slate-200/80 shadow-md hover:shadow-lg transition-all">
            <span className="text-xl font-black text-slate-950">Solar Power</span>
            <p className="text-xs text-slate-600 mt-0.5 font-bold">Panels &amp; Inverters</p>
          </div>
        </div>
